...
export class ServerDownloadFeedback {
    private downloadBar = new CLIProgress.SingleBar(
        { format: ` {bar} | {state} | chunk {value}/{total}`, clearOnComplete: false, noTTYOutput: true },
        CLIProgress.Presets.shades_classic
    )
    private finishDownloadBar: (() => void) | null = null
    private verifyBar = new CLIProgress.SingleBar(
        { format: ` {bar} | {state} | chunk {value}/{total}`, clearOnComplete: false, noTTYOutput: true },
        CLIProgress.Presets.shades_classic
    )
