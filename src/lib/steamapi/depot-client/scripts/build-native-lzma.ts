...
const source = join(root, "native", "steam_lzma.c")

const { platform, arch } = process
const target = `${platform}-${arch}`
const outDir = join(root, "native", target)

const ext = platform === "darwin" ? "dylib" : platform === "win32" ? "dll" : "so"
const out = join(outDir, platform === "win32" ? "steam_lzma.dll" : `libsteam_lzma.${ext}`)
...
console.log(`[depot-client] built ${relative(root, out)}`)
